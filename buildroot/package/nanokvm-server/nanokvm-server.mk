...
# nanokvm-server
#
# Builds the NanoKVM Go backend and React web UI from source.
# The built binary and web dist are consumed by nanokvm-sg200x.
#
################################################################################

NANOKVM_SERVER_VERSION = 2.4.0
...
# Audio and other unused libs stripped to reduce image size
...
define NANOKVM_SERVER_BUILD_CMDS
	...
	if [ ! -e $(@D)/$(NANOKVM_SERVER_GOMOD)/dl_lib/libopencv_video.so.409 ]; then \
		echo '' | $(TARGET_CC) -shared -Wl,-soname,libopencv_video.so.409 -nostdlib \
			-x c - -o $(@D)/$(NANOKVM_SERVER_GOMOD)/dl_lib/libopencv_video.so.409 ; \
	fi
	for cv_lib in libopencv_core libopencv_highgui libopencv_imgcodecs libopencv_imgproc libopencv_video ; do \
		if [ -e $(@D)/$(NANOKVM_SERVER_GOMOD)/dl_lib/$$cv_lib.so.409 -a \
		     ! -e $(@D)/$(NANOKVM_SERVER_GOMOD)/dl_lib/$$cv_lib.so ]; then \
			ln -s $$cv_lib.so.409 $(@D)/$(NANOKVM_SERVER_GOMOD)/dl_lib/$$cv_lib.so ; \
		fi ; \
	done
	...
	GOTOOLCHAIN=local GOPROXY=direct GOSUMDB="sum.golang.org" $(GO_BIN) mod tidy
	cd $(@D)/$(NANOKVM_SERVER_GOMOD) ; \
	...
	sed -i s/' -lvpu$$'/' -lvpu -lopencv_core -lopencv_highgui -lopencv_imgcodecs -lopencv_imgproc -lopencv_video'/ common/cgo.go
	if [ -e $(@D)/$(NANOKVM_SERVER_GOMOD)/dl_lib/libkvm_mmf.so ]; then \
		cd $(@D)/$(NANOKVM_SERVER_GOMOD) ; \
		sed -i s/'maixcam_lib'/'kvm_mmf'/g common/cgo.go ; \
	fi
	cd $(@D)/$(NANOKVM_SERVER_GOMOD) ; \
	CGO_ENABLED=1 $(NANOKVM_SERVER_GO_ENV) $(GO_BIN) build -x -ldflags="-extldflags '-Wl,-rpath,\$$ORIGIN/dl_lib -Wl,--allow-shlib-undefined -shared-libgcc'"
	cd $(@D)/web ; \
...
define NANOKVM_SERVER_INSTALL_TARGET_CMDS
	...
	rsync -r --verbose --links --safe-links --hard-links \
		--exclude='libopencv_*.so' \
		${@D}/server/dl_lib/ $(TARGET_DIR)/kvmapp/server/dl_lib/
